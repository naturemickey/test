...
def to_char    = {"0123456789abcdefghijklmnopqrstuvwxyz".charAt(to_integer(it))}
def to_array   = {p ->
    def a = [];
...
def to_string  = {s -> to_array(s).stream().map({""+to_char(it)}).collect(Collectors.joining())}

println(to_char(NUM10))
